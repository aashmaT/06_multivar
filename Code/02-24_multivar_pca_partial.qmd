...
Y is normally referred to as:  
  - Response variable: esp in experimental designs  
  - Dependent variable: 
  - Predicted variable  

X is normally referred to as:  
  - Explanatory variable: experimental designs  
  - Independent variable: independently measures that has connection with what we dependently measured.  
  - Predictor variable: machine learning framework 
...
Perhaps multiple Xs (predictors) without a Y (outcome): PCA. We have lots of X variables and we want to find common pattern or correlation between all the X variables. 
...
- Dimensionality reduction: When we have lots of colimns of X variables such that we want to select fewer variables that would explain more percentage of variation.
...
- Based on Y (outcome) existence:
    - Supervised: We give it the outcome variable.
    - Unsupervised: We just give data and based on similarities, the algorithm creates groups.
...
There are variables that are highly positively correlated and highly negatively correlated.

High correlation among multiple explanatory variables implies presence of multicollinearity issues.
...
weather %>%
  ...
  # value! = 1 removes the same pair correlation eg strength and strength
...
- Model estimates (slopes) magnitude and direction (+ or -) can change for multicollinear variables compared to a non-multicollinear model.  
...
weather %>%
  ...
  #stat_correlation()
```
We can see that these two variables are not correlated as we have a flat line.
...
The coefficient estimates remains same when we include both variables in the model.
...
```
The magnitude of estimates for mean_dayl.s_Apr turned negative (0.0009755533 to -0.010605077). Also the SE, statistic and p-value changed. 
...
lm_dayl.apr %>%
  ...
  mutate(mod=factor(mod,
                    levels=c("lm_dayl.apr",
                             "lm_prp.oct",
                             "lm_2var.uncor",
                             "lm_2var.cor"))) %>% #Rearranging the levels in order that we want as R just does alphabetical ordering.
...
When alone and with another uncorrelated variables, we can see that the slope of mean_dayl.s_April remains same but when modeled with another correlated variable, we can see that the magnitude and the direction changed. Also the SE is inflated.
...
- Is an unsupervised analysis (no outcome variable)  
...
```
2 dimensions: Longest eigen vector = PC1 and then the eigen vector length decrease as we move to other PC's.
...
## Choosing number of PCs  (PC: decrease dimensions, have dimensions that are uncorrelated)
...
This indicates that original variables were correlated (as we expected). As the percentage of explained variance decreases with increase in PC's, it indicated original vars were correlated.
...
All PC's are linear combination of all the variables and the coefficients signify the importance of that variable.
...
PC2:Summer months vp.pa have positive weights and summer months srad have negative weights.
...
Each arrow is for one variable. Long arrow in x-axis: More imp to Dim1. 
...
PC1 ~ sum_precip_oct * coefficient + mean_daylength_jan * coefficient
...
PCA created orthogonal dimensions so now the VIF = 1.
